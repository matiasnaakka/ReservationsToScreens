/**
 * @fileoverview Static file server for the infoScreenFrontendSvelte application.
 * Serves static files and handles single-page application routing.
 *
 * @module InfoScreenFrontendSvelteFileServer
 * @requires express
 * @requires http
 *
 * @security This server only serves static files from the 'infoScreenFrontendSvelte' directory
 ...
 * Configures Express to serve static files from 'infoScreenFrontendSvelte' directory
 * Files are served from a dedicated directory only
 */
app.use(express.static('infoScreenFrontendSvelte'));

/**
 * SPA Fallback Route
 * Handles all unmatched routes for single-page application
 * Only serves index.html from infoScreenFrontendSvelte directory
 */
app.get('*', (_req, res) => {
  try {
    res.sendFile('index.html', { root: 'infoScreenFrontendSvelte' });
  } catch (error) {
 ...
 * Listens on port 3002 for incoming connections
 */
http.listen(port, () => {
  logger.info(
    `infoScreenFrontendSvelte FILE SERVER started at: http://localhost:${port}/. Start time: ${startTime.toLocaleString()}`,
  );
});
