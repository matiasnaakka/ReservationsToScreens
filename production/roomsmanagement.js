/**
 * @fileoverview Static file server for the roomsmanagement application.
 * Serves static files and handles single-page application routing.
 *
 * @module RoomsManagementFileServer
 * @requires express
 * @requires http
 *
 * @security This server only serves static files from the 'roomsmanagement' directory
 ...
 * Only serves index.html from roomsmanagement directory
 */
app.get('*', (_req, res) => {
  try {
 ...
 * Listens on port 3004 for incoming connections
 */
http.listen(port, () => {
  logger.info(
